# Python Novice to Professional — Course Website

India's most comprehensive Python 3.13 course. Zero experience to production-grade Python engineering in 8 weekends.

## Hosting on GitHub Pages

1. Create a new GitHub repository: `python-zero-to-pro`
2. Upload all files from this folder to the repo root
3. Go to **Settings → Pages → Source → Deploy from branch → main → / (root)**
4. Your site will be live at: `https://yourusername.github.io/python-zero-to-pro/`

## Files

```
index.html        — Landing page (hero, curriculum overview, trainers, pricing)
curriculum.html   — Full 16-module curriculum with all topics and code examples
styles.css        — Shared styles (dark theme, typography, layout)
curriculum.css    — Curriculum page specific styles
main.js           — Animations, typewriter, scroll effects, TOC highlighting
README.md         — This file
```

## What's on the site

- **Hero** with animated code typewriter (rotates through dunder methods, async, decorators examples)
- **Curriculum overview** — all 4 pillars, 16 modules, with tags
- **Dunder methods showcase** — all 30+ magic methods with hover effects
- **Trainers section** — Sanjay Mahto & Praveen Kumar with contact numbers
- **Pricing** — Standard / Professional / Elite with ISA option
- **Full curriculum page** — every module with 10 topics, code examples, project exercise
- **Sticky TOC sidebar** on curriculum page with active highlight on scroll
- **Fully responsive** — works on mobile

## Course Details

| | |
|---|---|
| Duration | 8 weekends · 48 hours |
| Format | Live Saturday (3 hrs) + Sunday (3 hrs) |
| Audience | Complete beginners — zero experience required |
| Covers | Python 3.13 — OOP, dunder methods, collections, decorators, generators, sqlite3, threading, asyncio |
| Does NOT cover | ML, scikit-learn, model training (main AI Engineering course) |
| Start date | 20 April 2026 |

## Contact

- **Sanjay Mahto** — Senior AI Engineer · Optum · +91 77954 57284
- **Praveen Kumar** — AI Engineer · JP Morgan Chase · +91 62615 01868

## Customisation

- Change colours: edit `:root` variables in `styles.css`
- Add a new module: copy the `<div class="module" id="mXX">` block in `curriculum.html`
- Update pricing: edit the `.pricing-card` sections in `index.html`
- Change typewriter snippets: edit the `codeSnippets` array in `main.js`
# python-zero-to-professional
